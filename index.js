...
bot.on('text', async (ctx) => {
  ...
  const pendingAction = replyToMessageId ? db.getPendingAction(replyToMessageId.toString()) : null;

  if (pendingAction) {
    const { request_id: requestId, type, edit_message_id: messageIdToEdit, original_text: originalText } = pendingAction;
    ...
    if (type === 'AWAITING_DECLINE_REASON' && request.manager_status !== 'PENDING') {
      await ctx.reply("This request has already been processed.");
      db.deletePendingAction(replyToMessageId.toString());
      return;
    }
    if (type === 'AWAITING_ISSUE_REASON' && request.accountant_status !== 'PENDING') {
      await ctx.reply("This request has already been processed.");
      db.deletePendingAction(replyToMessageId.toString());
      return;
    }

    const { requester_id: manager1Id, manager_id: manager2Id, amount, recipient } = request;
    ...
    if (type === 'AWAITING_DECLINE_REASON') {
      ...
      }).catch(err => console.error("Sheets Sync Error (Update Manager):", err));
      ...
      }).catch(err => console.error("Sheets Sync Error (Update Accountant):", err));
      ...
      await ctx.reply("Issue reported to both managers.", { reply_markup: { remove_keyboard: true } });
    }
    ...
    } catch (e) { console.error("Failed to delete prompt message", e); }
  ...
  userContexts[userId].push({ role: 'user', content: userMessage });
    ...
    if (result.complete) {
      ...
      sheetsSync.appendRow(requestData).catch(err => console.error("Sheets Sync Error (Append):", err));
...
bot.action(/^(accept|decline)_(.+)$/, async (ctx) => {
  ...
  if (action === 'decline') {
    ...
    db.savePendingRequestEdit(requestId, ctx.callbackQuery.message.message_id.toString(), ctx.callbackQuery.message.text);
    ...
    for (const action of existingActions) {
      try {
        await bot.telegram.deleteMessage(action.user_id, parseInt(action.message_id));
    ...
    setTimeout(() => {
      bot.telegram.deleteMessage(ctx.chat.id, promptMessage.message_id)
        .then(() => db.deletePendingAction(promptMessage.message_id.toString()))
        .catch(() => db.deletePendingAction(promptMessage.message_id.toString()));
    }, 60000);
